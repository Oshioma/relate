-- =============================================================================
-- Relate — allow space_type = 'my_crops' at the CHECK-constraint level
--
-- Rebuilds spaces_space_type_check with the full set including 'my_crops'
-- (alongside 'crop_guides' and 'plant_scanner'), for deployments that guard the
-- column with a TEXT CHECK rather than the enum. Idempotent.
-- =============================================================================
...
alter table public.spaces add constraint spaces_space_type_check
  check (space_type::text in (
    ...
    'course', 'crop_guides', 'plant_scanner', 'my_crops'
  ));
