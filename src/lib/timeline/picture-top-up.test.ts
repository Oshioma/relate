...
test("across every seeded dataset, a record holding only its first picture is reported as short", async () => {
  ...
  // MEASURED, not guessed: the seeds carry 122 records with more than one
  // picture and 144 pictures beyond each record's first. It was 123 and 150
  // until twelve galleries that listed the SAME file twice were de-duplicated;
  // the floor moved down because the count was inflated, not because anything
  // was lost.
  ...
  assert.ok(shortRecords >= 122, `only ${shortRecords} records carry more than one picture`);
  assert.ok(shortPictures >= 144, `only ${shortPictures} pictures would be topped up`);
});
