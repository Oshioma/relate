...
export type AdminNavSection = { id: string; label: string };

// Sticky jump-nav for the (very long) admin page. Renders a horizontally
// scrollable row of chips, one per on-page section, and highlights the section
// currently under the nav via an IntersectionObserver scroll-spy. Mirrors the
// crop guide's CropSectionNav so the two long pages behave identically.
export function AdminSectionNav({ sections }: { sections: AdminNavSection[] }) {
  ...
  return (
    <nav
      ...
      className="sticky top-0 z-30 -mx-4 mb-8 border-b border-border bg-background/85 px-4 py-2 backdrop-blur supports-[backdrop-filter]:bg-background/70 sm:-mx-6 sm:px-6"
    >
